...
public_users.get("/isbn/:isbn", async (req, res) => {
  const userISBN = parseInt(req.params.isbn);
  const userBook = await books[userISBN];
  if (!userBook) {
...
public_users.get("/review/:isbn", function (req, res) {
  ...
  const userBook = books[userISBN];
  if (userBook) {
    return res.status(200).send(JSON.stringify(userBook.reviews, null, 4));
  } else {
    return res.status(404).json({ message: "ISBN not found." });
  }
});
